...
c
c  PREFATORY NOTE TO THE VERSION OF JANUARY 31, 2008:
c
c  A possibility has been added in this version to calculate vector components in the "Solar-
c  Wind GSM" coordinate system, also known as GSW (Geocentric Solar Wind), first introduced
c  by Hones et al., Planet. Space Sci., v.34, p.889, 1986 (aka GSWM, see Appendix, Tsyganenko
c  et al., JGRA, v.103(A4), p.6827, 1998). The GSW system is analogous to the standard GSM,
c  except that its X-axis is antiparallel to the currently observed solar wind flow vector,
c  rather than aligned with the Sun-Earth line. Therefore, the orientation of axes in the
c  Solar-Wind GSM system can be uniquely defined by specifying three components of the solar
c  wind flow, and in the case of a strictly radial anti-sunward flow that system reduces to
c  the standard GSM.  To avoid introducing more subroutines and minimize changes that could
c  result in confusion, it was decided to retain the name GSM for the Solar-Wind GSM system,
c  with a tacit understanding that its X-axis orientation can now be different from the
c  Earth-Sun line. To that end, the only modification in terms of users' interface was made
c  in the subroutine RECALC, by adding three more input parameters, namely, the concurrent
c  observed values of cartesian GSE components (VGSEX,VGSEY,VGSEZ) of the solar wind velocity.
C  Invoking RECALC with VGSEY=VGSEZ=0 restores the standard (sunward) orientation of the XGSM
C  axis, so that the GSM and SM (solar-magnetic) coordinate systems re-assume their standard
c  definition. For more details, see the documentation file GEOPACK-2008.DOC.
c
c  Another modification allows users to have more control over the procedure of field line
c  mapping using the subroutine TRACE. To that end, three new input parameters were added in
c  that subroutine, making it possible to set (i) an upper limit, DSMAX, on the automatically
c  adjusted step size, (ii) a permissible step error, ERR, and (iii) maximal length, LMAX,
c  of arrays where field line point coordinates are stored. Minor changes in the tracing
c  algorithm were also introduced, in order to (1) make it more compact and easier for
c  understanding, and (2) prevent the algorithm from making uncontrollable large number of
c  multiple loops in some cases with plasmoid-like field structures.
c
C  Finally, one more subroutine named GEODGEO was added to the package, allowing one to
c  convert geodetic coordinates of a point in space (altitude above the Earth's WGS84
c  ellipsoid and geodetic latitude) to geocentric radial distance and colatitude, and vice
c  versa.
C
C  For a complete list of modifications made in earlier versions, see the documentation
C  file GEOPACK-2008.DOC.
c
c----------------------------------------------------------------------------------
c
...
C  GSM" COORDINATE SYSTEM, USING IAGA INTERNATIONAL GEOMAGNETIC REFERENCE MODEL COEFFICIENTS
C  (e.g., http://www.ngdc.noaa.gov/IAGA/vmod/igrf.html, revised 22 March, 2005)
c
C  THE SOLAR-WIND GSM SYSTEM IS ESSENTIALLY SIMILAR TO THE STANDARD GSM (THE TWO SYSTEMS BECOME
C   IDENTICAL TO EACH OTHER IN THE CASE OF STRICTLY ANTI-SUNWARD SOLAR WIND FLOW). FOR A DETAILED
C   DEFINITION, SEE INTRODUCTORY COMMENTS FOR THE SUBROUTINE GSMGSE .
C
...
c  AND GEO-GSM ROTATION MATRIX ELEMENTS SHOULD BE UPDATED BY CALLING THE SUBROUTINE RECALC.
C
C-----INPUT PARAMETERS:
C
C     XGSM,YGSM,ZGSM - CARTESIAN SOLAR-WIND GSM COORDINATES (IN UNITS RE=6371.2 KM)
C
C-----OUTPUT PARAMETERS:
C
C     HXGSM,HYGSM,HZGSM - CARTESIAN SOLAR-WIND GSM COMPONENTS OF THE MAIN GEOMAGNETIC FIELD IN NANOTESLA
C
C     LAST MODIFICATION:  JAN 27, 2008.
...
C
C  CALCULATES SOLAR-WIND GSM COMPONENTS OF GEODIPOLE FIELD WITH THE DIPOLE MOMENT
...
C
C  THE SOLAR-WIND GSM COORDINATE SYSTEM IS ESSENTIALLY SIMILAR TO THE STANDARD GSM
c  (THE TWO SYSTEMS BECOME IDENTICAL TO EACH OTHER IN THE CASE OF STRICTLY RADIAL
c  ANTI-SUNWARD SOLAR WIND FLOW). ITS DETAILED DEFINITION IS GIVEN IN INTRODUCTORY
C  COMMENTS FOR THE SUBROUTINE GSMGSE .
...
c=====================================================================================
C
      SUBROUTINE RECALC (IYEAR,IDAY,IHOUR,MIN,ISEC,VGSEX,VGSEY,VGSEZ)
C
...
C
C  IMPORTANT: IF NO INFORMATION IS AVAILABLE ON THE SOLAR WIND SPEED, THEN SET
C             VGSEX=-400.0 AND VGSEY=VGSEZ=0. IN THIS CASE, THE SOLAR-WIND GSM COORDINATE
C             SYSTEM BECOMES IDENTICAL TO THE STANDARD GSM.
C
...
C
      ...
      DY3=DZ1*S2-DZ2*S1
C
C  NOW LET'S CALCULATE GEI COMPONENTS OF THE UNIT VECTOR X = EXGSM, DIRECTED ANTIPARALLEL
...
C
      ...
      DIP3=CT0
C
C  THIS ALLOWS US TO CALCULATE GEI COMPONENTS OF THE UNIT VECTOR Y = EYGSM
...
C
      ...
      Y3=Y3/Y
C
C   AND GEI COMPONENTS OF THE UNIT VECTOR Z = EZGSM = EXGSM x EYGSM = X x Y:
C
      Z1=X2*Y3-X3*Y2
      Z2=X3*Y1-X1*Y3
      Z3=X1*Y2-X2*Y1
C
C   ELEMENTS OF THE MATRIX GSE TO GSM ARE THE SCALAR PRODUCTS:
C
C  E11=(EXGSE,EXGSM)  E12=(EXGSE,EYGSM)  E13=(EXGSE,EZGSM)
C  E21=(EYGSE,EXGSM)  E22=(EYGSE,EYGSM)  E23=(EYGSE,EZGSM)
C  E31=(EZGSE,EXGSM)  E32=(EZGSE,EYGSM)  E33=(EZGSE,EZGSM)
C
...
C
C   GEODIPOLE TILT ANGLE IN THE GSM SYSTEM: PSI=ARCSIN(DIP,EXGSM)
C
      SPS=DIP1*X1+DIP2*X2+DIP3*X3
      CPS=SQRT(1.-SPS**2)
      PSI=ASIN(SPS)
C
C   ELEMENTS OF THE MATRIX GEO TO GSM ARE THE SCALAR PRODUCTS:
C
...
C  EXGSM=(X1,X2,X3),  EYGSM=(Y1,Y2,Y3),   EZGSM=(Z1,Z2,Z3)
C                                                           AND  THEREFORE:
C
      A11=X1*CGST+X2*SGST
      A12=-X1*SGST+X2*CGST
...
C   THEY ARE FOUND AS THE SCALAR PRODUCTS: CFI=GM22=(EYSM,EYMAG)=(EYGSM,EYMAG),
C                                          SFI=GM23=(EYSM,EXMAG)=(EYGSM,EXMAG),
...
C             0
C  NOW, NOTE THAT GEI COMPONENTS OF EYSM=EYGSM WERE FOUND ABOVE AS Y1, Y2, AND Y3,
...
C  COORDINATE SYSTEM AND THE "SOLAR-WIND GSM" (aka GSW, or GSWM), DEFINED AS FOLLOWS
C  (HONES ET AL., PLANET.SPACE SCI., V.34, P.889, 1986; TSYGANENKO ET AL., JGRA,
C  V.103(A4), P.6827, 1998):
C
C  IN THE SOLAR-WIND GSM SYSTEM, X AXIS IS ANTIPARALLEL TO THE OBSERVED DIRECTION OF
C  THE SOLAR WIND FLOW. TWO OTHER AXES, Y AND Z, ARE DEFINED IN THE SAME WAY AS FOR THE
C  STANDARD GSM, THAT IS, Z AXIS IS ORTHOGONAL TO X AXIS, POINTS NORTHWARD, AND LIES IN
C  THE PLANE DEFINED BY THE X- AND GEODIPOLE AXIS. THE Y AXIS COMPLETES THE RIGHT-HANDED
C  SYSTEM.
C
C  THE SOLAR-WIND GSM SYSTEM BECOMES IDENTICAL TO THE STANDARD GSM IN THE CASE OF
...
C
C   (1) BEFORE CALLING GSMGSE, BE SURE TO INVOKE SUBROUTINE RECALC, IN ORDER
...
C       TO DO THAT, SPECIFY THE LAST 3 PARAMETERS IN RECALC AS FOLLOWS:
...
C
C   (3) IF NO INFORMATION IS AVAILABLE ON THE SOLAR WIND SPEED, THEN SET VGSEX=VGSEY=VGSEZ=0.
C       IN THAT CASE, THE SOLAR WIND GSM COORDINATE SYSTEM BECOMES IDENTICAL TO THE STANDARD ONE.
C
...
C
      IF (J.GT.0) THEN
        XGSE=XGSM*E11+YGSM*E12+ZGSM*E13
        YGSE=XGSM*E21+YGSM*E22+ZGSM*E23
        ZGSE=XGSM*E31+YGSM*E32+ZGSM*E33
      ENDIF
C
C   INVERSE TRANSFORMATION: CARRIED OUT USING THE TRANSPOSED MATRIX:
C
      IF (J.LT.0) THEN
        XGSM=XGSE*E11+YGSE*E21+ZGSE*E31
        YGSM=XGSE*E12+YGSE*E22+ZGSE*E32
        ZGSM=XGSE*E13+YGSE*E23+ZGSE*E33
      ENDIF
C
      RETURN
      END
...
C  CAN SET VGSEX=-400.0, VGSEY=0.0, VGSEZ=0.0 IN RECALC.
C
...
C
C  ATTENTION:  SUBROUTINE  RECALC  MUST BE INVOKED BEFORE MAGSM IN THREE CASES:
...
C
C  NOTE: HERE WE DEFINE THE SOLAR MAGNETIC COORDINATES, ASSUMING THAT XGSM AXIS IS ALIGNED
C        ANTIPARALLEL TO THE OBSERVED SOLAR WIND FLOW, RATHER THAN ALONG EARTH-SUN LINE.
C        TO CONVERT MAG COORDINATES TO AND FROM THE STANDARD SM COORDINATES, INVOKE RECALC
C        WITH VGSEX=-400.0, VGSEY=0.0, VGSEZ=0.0
C
C     LAST MODIFICATION:  JAN 28, 2008
C
C     AUTHOR:  N. A. TSYGANENKO
...
C
C  CONVERTS SOLAR MAGNETIC (SM) TO GEOCENTRIC SOLAR-WIND MAGNETOSPHERIC (GSM)
C  COORDINATES OR VICE VERSA.
C
C                  J>0                 J<0
C-----INPUT: J,XSM,YSM,ZSM        J,XGSM,YGSM,ZGSM
C----OUTPUT:  XGSM,YGSM,ZGSM       XSM,YSM,ZSM
C
C  ATTENTION:  SUBROUTINE RECALC MUST BE INVOKED BEFORE SMGSM IN THREE CASES:
C     /A/  BEFORE THE FIRST TRANSFORMATION OF COORDINATES
C     /B/  IF THE VALUES OF IYEAR,IDAY,IHOUR,MIN,ISEC HAVE BEEN CHANGED
C     /C/  IF THE VALUES OF COMPONENTS OF THE SOLAR WIND FLOW VELOCITY HAVE CHANGED
C
C  NOTE: HERE WE DEFINE THE SM AND GSM COORDINATES, ASSUMING THAT XGSM AXIS IS ALIGNED
C        ANTIPARALLEL TO THE OBSERVED SOLAR WIND FLOW, RATHER THAN ALONG EARTH-SUN LINE.
C        TO CONVERT BETWEEN THE STANDARD SM AND GSM COORDINATE SYSTEMS, INVOKE RECALC
C        WITH VGSEX=-400.0, VGSEY=0.0, VGSEZ=0.0
C
C     LAST MODIFICATION:  JAN 28, 2008
C
...
C
C CONVERTS GEOGRAPHIC (GEO) TO GEOCENTRIC SOLAR-WIND MAGNETOSPHERIC (GSM) COORDINATES
C   OR VICE VERSA.
C
C                   J>0                   J<0
...
C
C  ATTENTION:  SUBROUTINE  RECALC  MUST BE INVOKED BEFORE GEOGSM IN THREE CASES:
C     /A/  BEFORE THE FIRST TRANSFORMATION OF COORDINATES, OR
C     /B/  IF THE VALUES OF IYEAR,IDAY,IHOUR,MIN,ISEC  HAVE CHANGED, AND/OR
C     /C/  IF THE VALUES OF COMPONENTS OF THE SOLAR WIND FLOW VELOCITY HAVE CHANGED
C
C  NOTE: THIS SUBROUTINE CONVERTS GEO VECTORS TO AND FROM THE SOLAR-WIND GSM COORDINATE
C        SYSTEM, TAKING INTO ACCOUNT POSSIBLE DEFLECTIONS OF THE SOLAR WIND DIRECTION FROM
C        STRICTLY RADIAL.  BEFORE CONVERTING TO/FROM STANDARD GSM COORDINATES, INVOKE RECALC
...
C=====================================================================================
C
      SUBROUTINE GEODGEO (H,XMU,R,THETA,J)
C
...
C     PARTS OF THE TOTAL FIELD, E.G., T96_01 AND IGRF_GSM
C
...
C===================================================================================
C
      SUBROUTINE STEP (X,Y,Z,DS,DSMAX,ERRIN,IOPT,PARMOD,EXNAME,INNAME)
C
C   RE-CALCULATES THE INPUT VALUES {X,Y,Z} (IN GSM COORDINATES) FOR ANY POINT ON A FIELD LINE,
...
C   ERRIN IS A PERMISSIBLE ERROR (ITS OPTIMAL VALUE SPECIFIED IN THE S/R TRACE)
...
C   INNAME IS THE NAME OF THE SUBROUTINE FOR THE INTERNAL FIELD MODEL (EITHER DIP OR IGRF_GSM)
C
...
C
C  (1) IF INNAME=IGRF_GSM, THEN THE IGRF MODEL WILL BE USED FOR CALCULATING
C      CONTRIBUTION FROM EARTH'S INTERNAL SOURCES. IN THIS CASE, SUBROUTINE
C      RECALC MUST BE CALLED BEFORE USING TRACE, WITH PROPERLY SPECIFIED DATE,
...
C
C  (2) IF INNAME=DIP, THEN A PURE DIPOLE FIELD WILL BE USED INSTEAD OF THE IGRF MODEL.
C      IN THIS CASE, THE SUBROUTINE RECALC MUST ALSO BE CALLED BEFORE TRACE.
...
C      ANGLE MUST BE SPECIFIED IN ADVANCE (BUT AFTER HAVING CALLED RECALC) AND FORWARDED
C      IN THE COMMON BLOCK /GEOPACK1/ (IN ITS 11th AND 12th ELEMENTS, RESPECTIVELY).
C      IN THIS CASE THE ROLE OF THE SUBROUTINE RECALC IS REDUCED TO ONLY CALCULATING
C      THE COMPONENTS OF THE EARTH'S DIPOLE MOMENT.
C
C------------- INPUT PARAMETERS:
C
C   XI,YI,ZI - GSM COORDS OF THE FIELD LINE STARTING POINT (IN EARTH RADII, 1 RE = 6371.2 km),
C
...
C-------------- OUTPUT PARAMETERS:
C
C   XF,YF,ZF - GSM COORDINATES OF THE ENDPOINT OF THE TRACED FIELD LINE.
...
c   coaxial with the XGSM axis, (3) the plane X=20Re:
...
c
      ...
      RR=R
c
      CALL STEP (X,Y,Z,DS,DSMAX,ERR,IOPT,PARMOD,EXNAME,INNAME)
c
...
C
C -----------------------------------------------------------------------------
C Subroutine SETPSI added by PJ 15.12.2003, copied from geopack2003.f 2.4.2008
C   but redimensioned BBB to 22 to conform new common block size
C -----------------------------------------------------------------------------
      SUBROUTINE SETPSI(PS)
      REAL PS
      COMMON /GEOPACK1/ AAA(10),SPS,CPS,BBB(22)
